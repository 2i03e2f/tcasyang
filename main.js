...
function _getProgLatLng(r) {
  ...
  const campus = uni.campuses.find(c => c.name === r.campus_name_th) ||
                 uni.campuses.find(c => c.location === r.campus_name_th) ||
                 uni.campuses[0];
  return campus ? { lat: campus.lat, lng: campus.lng } : null;
}
...
function renderPage(page, data) {
  const sorted = _sortData(data || _allResults);
  const list = sorted;
  const PER  = 20;
...
function applyFilters() {
  ...
  document.getElementById('sort-bar').style.display = _filteredCache.length ? 'flex' : 'none';
}
